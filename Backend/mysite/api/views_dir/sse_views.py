...
def dashboard_sse(request):
    ...
    def event_stream():
        ...
        initial_data = get_dashboard_data_sync(user, branch_id, role)
        if initial_data:
        ...
        try:
            while True:
                ...
                if has_dashboard_data_changed_sync(user, branch_id, last_check):
                    logger.debug(
                        f"Data changed for user {user.username}, sending update"
                    )
                    new_data = get_dashboard_data_sync(user, branch_id, role)
                    if new_data:
...
def get_global_dashboard_data(today):
    ...
    # Get recent activity - convert Decimal fields
    recent_invoices = list(
        Invoice.objects.select_related("branch", "created_by")
        .order_by("-created_at")[:5]
        .values(
            "id", "total_amount", "created_at", "branch__name", "created_by__username"
        )
    )
    ...
    for invoice in recent_invoices:
        ...
        if "created_at" in invoice and invoice["created_at"] is not None:
    ...
    return {
        ...
        "total_user": total_user_count - 1,  # Exclude superuser?
...
def get_branch_dashboard_data(branch_id, today, yesterday):
    ...
    # Get top selling items for this branch - convert quantities to int
    top_selling_items = list(
    ...
    # Clean up top selling items and ensure quantities are int
    top_selling_items = [
        {
            "product__name": item.get("product__name") or "Unknown",
            "total_quantity": int(item["total_quantity"] or 0),  # Convert to int
        }
        for item in top_selling_items
    ]

    # Get sales by category - using calculated line_total with F expressions
    ...
    # Clean up category data and convert to float
    sales_by_category = [
        {
            "product__category__name": item.get("product__category__name")
            or "Uncategorized",
            "total_sales": float(item["total_sales"] or 0),  # Convert to float
        }
        for item in sales_by_category
    ]

    # Get recent orders for this branch - using created_by instead of user
    recent_orders = list(
        Invoice.objects.filter(branch_id=branch_id)
        .select_related("created_by")
        .order_by("-created_at")[:5]
        .values("id", "total_amount", "created_at", "created_by__username")
    )

    # Convert Decimal fields in recent_orders to float and format datetime
